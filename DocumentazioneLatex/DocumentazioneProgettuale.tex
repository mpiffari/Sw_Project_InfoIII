...
\end{figure}
\noindent
In figura 3.1 e 3.2 sono mostrati il Deployment Diagram e l'Architecture Envisioning del sistema progettato per lo sviluppo dell’applicazione di Book Crossing. Si può osservare che si tratta di un’architettura \textbf{\textit{Three Tiers}}:
\begin{enumerate}
...
\end{figure}
\newpage
Nella figura 4.1 è mostrata l'architettura software modellizzata attraverso una rete di Petri. Innanzitutto si può già osservare chè è stata definita seguendo il modello architetturale MVC:
\begin{itemize}
...
\end{figure}
\noindent
In figura 4.2 e 4.3 è mostrata la Logical View del sistema progettato. Si può osservare che segue il modello definito attraverso il pattern archietteturale Model View Controller/Presenter, dal momento che vengono individuati tre strati, ciascuno dei quali con le seguenti caratteristiche:
\begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\\\\\\\\\\\\
		Quindi, a livello di pseudo-codice, possiamo scrivere il nostro algoritmo come
	
		\begin{lstlisting}
		if VerificaPuntoIncontro(Lettore, Prenotante) {
			 Ricerca possibile punto di incontro tra i due utenti
			 Notifica di dove effettuare lo scambio		
		} else {
			 Trovare la rete di users che stanno nel mezzo tra i due utenti 
			 Ricerca del cammino ottimo passando attraverso gli utenti intermedi
		}
		\end{lstlisting}
		
		Nello specifico, la funzionalità \textit{VerificaPuntoIncontro(Lettore, Prenotante)} andrà a verificare che:
		...
		Nel caso in cui i due utenti non abbiano un possibili punto d'incontro, dobbiamo selezionare gli utenti tramite il quale il libro in questione potrà viaggiare: l'idea base è quindi quella di costruirsi un'area circolare il centro si trovi nella metà della congiungente del punto $ z_{0} $ (zona di residenza lettore) e $ z^{p}_{i} $ (zona di residenza prenotante).
